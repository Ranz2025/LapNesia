...
import {
  ...
  ArrowUpRight, ArrowDownLeft, RefreshCw, Receipt, Wrench
} from "lucide-react";
...
/* Label tipe transaksi yang lebih ramah dibaca */
const TYPE_LABEL = {
  inspection_income:         "Pendapatan Inspeksi",
  inspection_income_release: "Pencairan Inspeksi",
  inspection_fee:            "Biaya Inspeksi",
  sale_income:               "Pendapatan Penjualan",
  escrow_release:            "Pencairan Escrow",
  withdraw:                  "Penarikan Dana",
  refund:                    "Refund",
  platform_fee_adjustment:   "Penyesuaian Biaya",
  freeze_for_withdrawal:     "Dana Dibekukan",
  release_freeze:            "Dana Dicairkan",
};

const TXN_CREDIT_TYPES = ["inspection_income", "inspection_income_release", "sale_income", "escrow_release", "refund", "release_freeze"];

const TXN_STATUS_STYLE = {
  escrow:    { background: "rgba(217,119,6,0.10)",  color: "#D97706", border: "rgba(217,119,6,0.25)",  label: "Ditahan" },
  released:  { background: "rgba(5,150,105,0.10)",  color: "#059669", border: "rgba(5,150,105,0.25)",  label: "Dicairkan" },
  completed: { background: "rgba(5,150,105,0.10)",  color: "#059669", border: "rgba(5,150,105,0.25)",  label: "Selesai" },
  refunded:  { background: "rgba(239,68,68,0.10)",  color: "#DC2626", border: "rgba(239,68,68,0.25)",  label: "Refund" },
  pending:   { background: "rgba(217,119,6,0.10)",  color: "#D97706", border: "rgba(217,119,6,0.25)",  label: "Pending" },
};
      ...
      <main className={`${SECTION_X} py-8`}>
        ...
        >
          ...
          <div className="relative">
            <div className="mb-2">
              <SectionLabel icon={<Wrench size={11} />} text="Teknisi Dashboard" />
            </div>
            ...
            <p className="text-sm" style={{ color: CLR_MUTED }}>
              Pantau pendapatan jasa inspeksi dan cairkan saldo kamu.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              ...
              />
              <WalletCard
                icon={Lock} label="Saldo Ditahan (Escrow)"
                value={`Rp ${Number(wallet.held_balance || 0).toLocaleString("id-ID")}`}
                sub="Menunggu pekerjaan selesai"
            ...
            {/* ── Keterangan alur uang ── */}
            <div
              className="rounded-2xl p-4 mb-6 flex items-start gap-3"
              style={{ background: "rgba(37,99,235,0.05)", border: `1px solid ${CLR_BORDER}` }}
            >
              <Sparkles size={16} style={{ color: CLR_ACCENT, flexShrink: 0, marginTop: 2 }} />
              <p className="text-sm" style={{ color: CLR_MUTED }}>
                <span className="font-semibold" style={{ color: CLR_TEXT }}>Alur pendapatan:</span>{" "}
                Setelah buyer membayar, saldo masuk ke <span className="font-medium" style={{ color: "#D97706" }}>Saldo Ditahan (Escrow)</span>.
                Saldo akan otomatis pindah ke <span className="font-medium" style={{ color: "#059669" }}>Saldo Tersedia</span> setelah kamu menandai pekerjaan sebagai selesai.
              </p>
            </div>
              ...
              {!Array.isArray(transactions) || transactions.length === 0 ? (
                <div className="px-6 py-14 text-center">
                  ...
                  <p className="text-xs mt-1" style={{ color: CLR_SUBTLE }}>
                    Pendapatan akan muncul di sini setelah buyer membayar inspeksi.
                  </p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-sm min-w-[600px]">
                    ...
                    <tbody>
                      {transactions.map((txn, idx) => {
                        const isCredit   = TXN_CREDIT_TYPES.includes(txn.type);
                        const amtColor   = isCredit ? "#059669" : "#DC2626";
                        const amtBg      = isCredit ? "rgba(5,150,105,0.10)"  : "rgba(239,68,68,0.10)";
                        const AmtIcon    = isCredit ? ArrowDownLeft : ArrowUpRight;
                        const sign       = isCredit ? "+" : "-";
                        const statusStyle = TXN_STATUS_STYLE[txn.status] || { background: "rgba(100,116,139,0.10)", color: CLR_MUTED, border: "rgba(100,116,139,0.20)", label: txn.status };
                        ...
                        return (
                          ...
                          >
                            <td className="px-6 py-3.5 whitespace-nowrap" style={{ color: CLR_MUTED }}>
                              {new Date(txn.created_at).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })}
                            </td>
                            <td className="px-6 py-3.5">
                              <span className="inline-flex items-center gap-1.5 font-medium" style={{ color: CLR_TEXT }}>
                                <span className="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: amtBg }}>
                                  <AmtIcon size={12} style={{ color: amtColor }} />
                                </span>
                                {TYPE_LABEL[txn.type] || txn.type}
                              </span>
                            </td>
                            <td className="px-6 py-3.5 text-[13px] max-w-[200px] truncate" style={{ color: CLR_MUTED }}>
                              {txn.description || "-"}
                            </td>
                            <td className="px-6 py-3.5 font-semibold whitespace-nowrap" style={{ color: amtColor }}>
                              {sign} Rp {Number(txn.amount).toLocaleString("id-ID")}
                            </td>
                            <td className="px-6 py-3.5">
                              <span
